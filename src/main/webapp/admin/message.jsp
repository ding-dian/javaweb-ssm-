...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
           ...
           &nbsp;&nbsp;&nbsp;
            邮箱：<input name="uemail" id="uemail">
            ...
            <c:when test="${info.list.size()!=0}">
                ...
                <div id="middle">
                    <table class="table table-bordered table-striped">
                        <tr>
                            <th></th>
                            <th style="text-align:center;vertical-align:middle;">用户名字</th>
                        ...
                        <c:forEach items="${info.list}" var="p">
                            <tr>
                                ...
                                <td>${p.uName}</td>
                                <td width="13%" height="70">${p.uEmail}</td>
                                <td <%--style="word-wrap:break-word;"--%>width="65%" height="70">
                                    <div style="width:100%;height:100%;white-space:normal;word-wrap:break-word;word-break:break-all;overflow-y:scroll;"<%--style="overflow-x:scroll;overflow-y:scroll;white-space:pre-wrap"--%> >
                                            ${p.uMessage}
                                    </div>
                                </td>
                                <%--<td>${p.uMessage}}</td>--%>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${p.uid},${info.pageNum})">回复
                                    </button>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="del(${p.uid},${info.pageNum})">删除
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
...
<script type="text/javascript">
        ...
        if(cks.length == 0){
            ...
            if(confirm("您确定要删除"+cks.length+"条商品吗?")){
               // alert("可以进行删除!");
                ...
                //发送ajax请求,进行批量删除的提交
                $.ajax({
                    url:"${pageContext.request.contextPath}/prod/deleteBatchmes.action",
                    data:{"pids":str},
                    type:"post",
                    dataType:"text",
                    success:function (msg) {
                        ...
                        //将页面上显示商品数据的容器重新加载
                        $("#table").load("http://localhost:8080/admin/message.jsp #table");
                        $("#table").load("http://120.77.32.127:8080/webssm/admin/message.jsp #table");
                    }
                });
            }
        }
    ...
    function del(uid) {
        ...
        if (confirm("您确定删除吗?")) {
            //发出ajax的请求,进行删除操作
            $.ajax({
                url: "${pageContext.request.contextPath}/prod/deletemes.action",
                data: {"uid": uid},
                type: "post",
                dataType: "text",
                success: function (msg) {
...
<script type="text/javascript">
    function ajaxsplit(page) {
       /* //取出查询条件
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/prod/ajaxSplitmes.action",

           // data: {"page": page,"pname":pname,"typeid":typeid,"lprice":lprice,"hprice":hprice},
            data:{"page":page,"uname":uname,"uemail":uemail},
            type: "post",
            success: function () {
                //重新加载显示分页数据的容器
                $("#table").load("http://localhost:8080/admin/message.jsp #table");
    ...
    function condition() {
        ...
        $.ajax({
            type:"post",
           // url:"${pageContext.request.contextPath}/prod/conditionmes.action",
            url:"${pageContext.request.contextPath}/prod/ajaxSplitmes.action",
            data:{"uname":uname,"uemail":uemail},
            success:function () {
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/admin/message.jsp #table");
                $("#table").load("http://120.77.32.127:8080/webssm/admin/message.jsp #table");
